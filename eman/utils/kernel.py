...
def create_self_kernel_basis(l_in, l_out, band_limit=None):
    """
    ...
    """
    # same code as create_kernel_basis , with a few changes to ensure the basis is for self kernel
    if l_in == 0 and l_out == 0:
        bases = np.ones((1, 1, 1, 1))
        frequencies = [0]
        theta_freq_dims = [0]
    elif l_in == 0 and l_out != 0:
        bases = np.array([[0, 0, 0, 0], [0, 0, 0, 0]]).reshape((2, 2, 2, 1))
        frequencies = [l_out, l_out]
        theta_freq_dims = [2 * l_out - 1, 2 * l_out]
    elif l_in != 0 and l_out == 0:
        bases = np.array([[0, 0, 0, 0], [0, 0, 0, 0]]).reshape((2, 1, 2, 2))
...
def build_self_kernel(in_order, out_order, band_limit=None):
    """
    ...
    for l_in, l_out in product(range(in_order + 1), range(out_order + 1)):
        bases, theta_freq_dims, frequencies = create_self_kernel_basis(
