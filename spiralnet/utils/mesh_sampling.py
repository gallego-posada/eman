...
def setup_deformation_transfer(source, target, use_normals=False):
    ...
    nearest_faces, nearest_parts, nearest_vertices = source.compute_aabb_tree().nearest(
        target.v, True
    )
    ...
    for i in range(target.v.shape[0]):
        ...
        if n_id == 0:
            ...
            A = np.vstack(
                (source.v[nearest_f[n_id - 1]], source.v[nearest_f[n_id % 3]])
            ).T
    ...
    matrix = sp.csc_matrix(
        (coeffs_v, (rows, cols)), shape=(target.v.shape[0], source.v.shape[0])
    )
    return matrix
