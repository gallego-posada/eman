...
def test_conv_gauge_equivariance():
    ...
    conv = GemConv(*channels, in_order, out_order, n_rings=n_rings, equiv_bias=True).to(
        dtype
    )
    ...
    x_c = conv(x, data.edge_index, data.precomp.double(), data.connection)
    x_c_t = rep_act(x_c, -transform_angle)

    x_t = rep_act(x, -transform_angle)
    x_t_c = conv(x_t, data_t.edge_index, data_t.precomp.double(), data_t.connection)

    assert torch.allclose(x_c_t, x_t_c, atol=1e-7)
